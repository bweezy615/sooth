...
def _series(rows: Iterable[dict]) -> dict[tuple, list[dict]]:
    """One price history per (event, market, selection, book)."""
    out: dict[tuple, list[dict]] = defaultdict(list)
    for r in rows:
        key = (str(r.get("event_id")), str(r.get("market")),
               str(r.get("selection")), str(r.get("book")))
        out[key].append(r)
    for k in out:
...
def find_drift(rows: list[dict], min_move: float = DEFAULT_MIN_MOVE) -> list[Alert]:
    ...
    """
    alerts: list[Alert] = []
    for (event_id, market, selection, book), obs in _series(rows).items():
...
def find_divergence(rows: list[dict], min_move: float = DEFAULT_MIN_MOVE) -> list[Alert]:
    ...
    for (event_id, market, selection, _), r in latest.items():
        by_selection[(event_id, market, selection)].append(r)

    alerts: list[Alert] = []
    for (event_id, market, selection), quotes in by_selection.items():
